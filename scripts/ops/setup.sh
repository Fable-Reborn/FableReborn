...
"
cp data/db/schema.sql schematmp.sql
...
cp $IDLEDIR/config/examples/config.py config.py
mv $IDLEDIR/init.sh $BASEDIR/pginit/init.sh
cp $IDLEDIR/config/ops/redis/redis.conf $BASEDIR/redisconf/redis.conf
...
cp $IDLEDIR/config/ops/systemd/*.service $BASEDIR/.config/systemd/user/
