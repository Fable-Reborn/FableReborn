...
from fable.utils.checks import AlreadyRaiding, has_char, is_gm, is_god
...
class Raid(commands.Cog):
    ...
    async def auto_spawn_raid(self, channel, hp, rarity="magic", raid_hp=17776):
        ...
        try:
            ...
            # Create embed
            fi = discord.File("assets/images/misc/startdragon.webp")
            em = discord.Embed(
            ...
            fi_path = "assets/images/misc/startdragon.webp"
            try:
    ...
    async def celestialvault(self, ctx):
        ...
        try:
            ...
            fi = discord.File("assets/images/misc/celestialguardian.webp")
            em = discord.Embed(
            ...
            try:
                for channel_id in channels_ids:
                    try:
                        channel = self.bot.get_channel(channel_id)
                        if channel:
                            fi = File("assets/images/misc/celestialguardian.webp")
                            sent_msg = await channel.send(embed=em, file=fi, view=view)
                            message_ids.append(sent_msg.id)
                    except Exception as e:
                        error_message = f"Error in channel with ID {channel_id}: {e}. continuing.."
