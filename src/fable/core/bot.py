...
from fable.core.bucket_cooldown import Cooldown, CooldownMapping
from fable.core.classes import Mage, Paragon, Raider, Ranger, Ritualist, Thief, Warrior, Paladin, Reaper, Tank
from fable.core.classes import from_string as class_from_string
from fable.core.context import Context
from fable.core.enums import DonatorRank
from fable.core.exceptions import GlobalCooldown
from fable.core.http import ProxiedClientSession
from fable.core.items import ALL_ITEM_TYPES, Hand, ItemType
from fable.utils import i18n, paginator, random
from fable.utils.cache import cache
from fable.utils.checks import user_is_patron
from fable.config import ConfigLoader
...
class Bot(commands.AutoShardedBot):
    ...
    async def setup_hook(self):
        ...
        for extension in self.config.bot.initial_extensions:
            if extension.startswith("cogs."):
                extension = extension.replace("cogs.", "fable.systems.", 1)
            try:
    ...
    async def get_raidstatsjug(
        ...
        """Generates the raidstats for a user"""
        from fable.systems.tournament import Tournament
        v = thing.id if isinstance(thing, (discord.Member, discord.User)) else thing
        local = False
        if conn is None:
            conn = await self.pool.acquire()
